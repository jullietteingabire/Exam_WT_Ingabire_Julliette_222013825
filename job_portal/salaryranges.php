...
<center>
<h5> SALARYRANGES INFORMATION</h5>
<form  method="POST" style="background-color: WHITE; align-items: center; width: 300px; height: 250px;"> <br><br><br>
	    <label>Enter salary_range_id:</label><br>
        <input type="number" name="id" required><br>

        <label>Enter minimum_salary:</label><br>
        <input type="number" name="minimum_salary" required><br>

        <label>Enter maximum_salary:</label><br>
        <input type="number" name="maximum_salary" required><br>

        <label>Enter currency:</label><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO salaryranges (salary_range_id, minimum_salary,maximum_salary,currency) VALUES (?,?,?,?)");
    $stmt->bind_param("ssss", $id, $minimum_salary,$maximum_salary,$currency);
...
<body>
    <h2>TABLE OF salaryranges DATA</h2>
    
    <table id="dataTable">
        <tr>
            <th>salary_range_id</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $salary_range_id = $row['salary_range_id'];
                echo "<tr><td>" . $row["salary_range_id"] .
                     "</td><td>" . $row["minimum_salary"] .
                      "</td><td>" . $row["maximum_salary"] .
                       "</td><td>" . $row["currency"] .
                      "<td><a style='padding:4px' href='salaryrangesupdate.php?salary_range_id=$salary_range_id'>update</a></td> 
                    <td><a style='padding:4px' href='salaryrangesdelete.php?salary_range_id=$salary_range_id'>delete</a></td> 
                </tr>";     
            }
        } else {
