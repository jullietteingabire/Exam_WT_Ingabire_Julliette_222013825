...
<center>
<h5>APPLICATIONS INFORMATION</h5>
<form method="POST" class="sp" onsubmit="return confirmInsert()">
    <label>Enter application_id:</label><br>
    <input type="number" name="application_id" required><br>

    <label>Enter application_date:</label><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO applications (application_id, application_date,candidate_id,payment) VALUES (?,?,?,?)");
    $stmt->bind_param("ssss", $application_id, $application_date,$candidate_id,$payment);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of applications</title>
    <style>
        table {
            width: 100%;
...
<body>
    <h2>TABLE OF APPLICATIONS DATA</h2>
                ...
                echo "<tr><td>" . $row["application_id"] .
                     "</td><td>" . $row["application_date"] .
                      "</td><td>" . $row["candidate_id"] .
                       "</td><td>" . $row["payment"] .
                       "<td><a style='padding:4px' href='applicationsupdate.php?application_id=$application_id'>update</a></td> 
                    <td><a style='padding:4px' href='applicationsdelete.php?application_id=$application_id'>delete</a></td> 
                </tr>";     
            }
        } else {
