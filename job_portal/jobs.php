...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO jobs (job_id, title,description,location,company_id) VALUES ( ?,?,?,?,?)");
    $stmt->bind_param("sssss", $job_id, $title, $description, $location, $company_id);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of jobs</title>
    <style>
        table {
            width: 100%;
                ...
                echo "<tr><td>" . $row["job_id"] .
                     "</td><td>" . $row["title"] .
                      "</td><td>" . $row["description"] .
                       "</td><td>" . $row["location"] .
                        "</td><td>" . $row["company_id"] .
                        

                      "<td><a style='padding:4px' href='jobsupdate.php?job_id=$job_id'>update</a></td> 
                    <td><a style='padding:4px' href='jobsdelete.php?job_id=$job_id'>delete</a></td> 
                </tr>";     
            }
        } else {
