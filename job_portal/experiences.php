...
<center>
<h5> EXPERIENCES INFORMATION</h5>
<form  method="POST" style="background-color: WHITE; align-items: center; width: 300px; height: 350px;"> <br><br><br>
        <label>Enter experience_id:</label><br>
        <input type="number" name="experience_id" required><br>
        ...
        <input type="date" name="start-date" required><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO experiences(experience_id, company,position,start_date,end_date,candidate_id) VALUES ( ?,?,?,?,?,?)");
    $stmt->bind_param("issssi", $id, $company, $position, $start_date, $end_date, $candidate_id);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of experiences</title>
    <style>
        table {
            width: 100%;
...
<body>
    <h2>TABLE OF EXPERIENCES DATA</h2>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr><td>" . $row["experience_id"] .
                     "</td><td>" . $row["company"] .
                      ...
                      "<td><a style='padding:4px' href='experiencesupdate.php?experience_id=$experience_id'>update</a></td> 
                    <td><a style='padding:4px' href='experiencesdelete.php?experience_id=$experience_id'>delete</a></td> 
                </tr>";     
            }
        } else {
